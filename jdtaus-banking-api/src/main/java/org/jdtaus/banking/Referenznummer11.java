...
public final class Referenznummer11 extends Number implements Comparable
{
     ...
     * <p>The electronic format of a Referenznummer11 is an eleven digit number with leading zeros omitted
     * (e.g. 6789).</p>
     */
     ...
     * <p>The letter format of a Referenznummer11 is an eleven digit number with leading zeros omitted separated by
     * spaces between the first three digits and the second three digits, the second three digits and the third three
     ...
     *
     * @throws IllegalArgumentException if {@code referenceCode} is negative, zero or greater than 99999999999.
     *
     ...
     *
     * @param referenceCode A Referenznummer11 in either electronic or letter format.
     ...
     */
    public static Referenznummer11 parse( final String referenceCode, final ParsePosition pos )
    {
    ...
    /**
     * Parses text from the beginning of the given string to produce a {@code Referenznummer11}.
     ...
     */
    public static Referenznummer11 parse( final String referenceCode ) throws ParseException
    {
        if ( referenceCode == null )
        {
            throw new NullPointerException( "referenceCode" );
        ...
        Referenznummer11 ref = (Referenznummer11) getCache().get( referenceCode );
     ...
     *
     * @return {@code true} if {@code referenceCode} is a valid Referenznummer11; {@code false} if not.
     */
    ...
    public StringBuffer format( final int style, final StringBuffer toAppendTo )
    {
        ...
        if ( style != Referenznummer11.ELECTRONIC_FORMAT && style != Referenznummer11.LETTER_FORMAT )
        {
        ...
        else
        {
            ...
            for ( int i = digits.length - 1, lastDigit = 0; i >= 0; i-- )
            {
                ...
                if ( style == Referenznummer11.LETTER_FORMAT && ( lastDigit == 3 || lastDigit == 6 || lastDigit == 9 ) )
                {
                    toAppendTo.append( ' ' );
                }
            }
